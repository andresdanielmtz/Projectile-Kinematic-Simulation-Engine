...
xmax = 250^2 * sind(angulo(i)) / abs(g);
ymax = (250^2 * sind(angulo(i)) ^ 2/abs(2*g));
...
%   end 
  ...
  a = text(xmax, 0.1 * ymax, text_xmax);
  text_ymax = sprintf("y_{max} = %.2f m", ymax + 1763);
  b = text(xmax / 3, ymax * 2.4, text_ymax);
